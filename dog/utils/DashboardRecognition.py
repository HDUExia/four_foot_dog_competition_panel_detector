...
class DashboardRecognition:
    ...
    def visualize(self, image, thickness=3):
        if self.bbox is not None:
            ...
            if self.dashboard_status is not None:
                cv.putText(image, self.dashboard_status, (self.bbox[0][0] - 30* thickness, self.bbox[0][1] - 30*thickness ),
                           cv.FONT_HERSHEY_SIMPLEX,thickness, (0, 0, 255),int(thickness))
        return image
